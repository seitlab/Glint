...
class TAGConvWithDropout(TAGConv):
    """
    A TAGConv followed by a Dropout.
    """
    def __init__(self, in_feats, out_feats, dropout=0.5, norm='both', weight=True, 
                 bias=True, activation=None, allow_zero_in_degree=False):
        super(TAGConvWithDropout, self).__init__(in_feats, out_feats)
        self.dropout = torch.nn.Dropout(p=dropout)

    def call(self, graph, feat, weight=None):
        feat = self.dropout(feat)
        return super(TAGConvWithDropout, self).call(graph, feat, weight)
...
class IndexSelect(torch.nn.Module):
    ...
    def __init__(self, pool_ratio:float, hidden_dim:int,
        ...
        self.gcn = TAGConvWithDropout(hidden_dim, hidden_dim)
...
class GraphPool(torch.nn.Module):
    ...
    def __init__(self, hidden_dim:int, use_gcn=False):
        ...
        self.down_sample_gcn = TAGConvWithDropout(hidden_dim, hidden_dim) \
...
class GraphUnpool(torch.nn.Module):
    ...
    def __init__(self, hidden_dim:int):
        ...
        self.up_sample_gcn = TAGConvWithDropout(hidden_dim, hidden_dim)
